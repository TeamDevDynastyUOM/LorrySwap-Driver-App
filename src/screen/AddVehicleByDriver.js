...
const AddVehicleByDriver = (props) => {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
        setLoading(true);  
        const addVehicleResponse = await fetch(`${BASE_URL}/owner/add_vehicle/${id}`, {
          method: 'POST',
          headers: {
        ...
        console.log('Response from add_vehicle:', addVehicleData);
    
        if (!addVehicleResponse.ok || !addVehicleData.success) {
          throw new Error(addVehicleData.error || 'Failed to add vehicle');
        }

        const editVehicleResponse = await fetch(`${BASE_URL}/owner/edit_vehicle/${id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
          },
          body: JSON.stringify({ vehicle_no: values.no }),
        });
    
        const editVehicleData = await editVehicleResponse.json();
        console.log('Response from edit_vehicle:', editVehicleData);
    
        if (!editVehicleResponse.ok) {
        ...
        console.error('Error handling vehicle operations:', error);
    ...
    <View style={styles.wrapper}>
      ...
      <View style={styles.formcontainer}>
        ...
        <ButtonContained buttonName="Add Vehicle " 
            onPress={() => handleSubmit()} iconName="truck" 
...
const styles = StyleSheet.create({
 ...
 export default AddVehicleByDriver;
